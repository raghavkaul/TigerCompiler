		...
		while(firstSetchanged){
			...
			for(Rule rule: rules){
				...
				if(RuleFirstSet.containsAll(rhs)){
					...
					//Else add everything to RuleFirstSet.
...
//		for(String nt: firstset.keySet()) {
//			Set<Rule> ntrs = new HashSet<Rule>();
//			for (Rule rule : rules)
//				if (rule.getName().equals(nt))
//					ntrs.add(rule);
//			for(Rule rule: ntrs){
//				Set<String> rulefirstset = firstset.get(rule.getName());
//				if(!rulefirstset.contains(TokenType.NIL.toString())){
//					for(String terminal:rulefirstset){
//						parsertable.put(rule.getName() + ", " + terminal, rule);
//					}
//				}
